...
class IPPanelAPI {
  ...
  async confirmOtp(token: string, otp: string) {
    return this.request('/api/acl/auth/confirm_otp', { method: 'POST', body: JSON.stringify({ token, otp }) });
  }
...
// ===== Login Page =====
function LoginPage({ login }: { login: (apiKey: string) => Promise<boolean> }) {
  ...
  return (
    <div className="px-4 pt-6 pb-28 space-y-6">
      ...
      <section className="animate-slide-up">
        ...
        <div className="grid grid-cols-2 gap-3">
          {modes.map((mode) => (
            <button key={mode.id} onClick={() => toast.success(`صفحه ${mode.title} به زودی اضافه می‌شود`)} className="card card-interactive p-4 flex flex-col items-center text-center group">
...
// ===== Profile Page =====
function ProfilePage({ userInfo, credit, numbers, accounts, currentAccountId, logout, switchAccount, addAccount, removeAccount, renameAccount }: any) {
  ...
  const handleAdd = async () => {
    ...
    setIsAdding(true);
    const success = await addAccount(newName.trim(), newKey.trim());
    if (success) {
